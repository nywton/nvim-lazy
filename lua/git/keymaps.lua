...
local review = require("git.review")
...
vim.keymap.set("n", "<leader>gv", function() diffsplit.split() end, { desc = "Side-by-side diff vs HEAD" })
vim.keymap.set("n", "<leader>gV", diffsplit.pick, { desc = "Side-by-side diff vs picked revision (fzf)" })
vim.keymap.set("n", "<leader>gr", review.start, { desc = "Repo-wide diff review (C-n/C-p hunk/file, gs stage hunk, q quit)" })
...
vim.keymap.set("n", "<leader>gl", history.start, { desc = "Git log -p history browser (C-n/C-p file/commit, C-d/C-u page, q quit)" })
...
vim.keymap.set("n", "<leader>gS", git.unstage_file, { desc = "Git unstage current file" })
